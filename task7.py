...
if __name__ == "__main__":
    ...
    parking_v = lines_parking.map(lambda x: (x[p_header.index('violation_code')], (1 / weekend_days, 0)) if x[p_header.index('issue_date')] in weekends
                                  else (x[p_header.index('violation_code')], (0, 1 / week_days)))

    parking_v = parking_v.reduceByKey(lambda x, y: (x[0] + y[0], x[1] + y[1]))

    out = parking_v.map(lambda x: '{0}\t{1:.2f}, {2:.2f}'.format(x[0], x[1][0], x[1][1]))
